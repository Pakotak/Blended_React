...
function getLastElement<T>(container: T[]): T  { 
    return container[container.length - 1];
}

const lastNumber = getLastElement<number>(numberContainer.items);
const lastString = getLastElement<string>(stringContainer.items);

console.log("1", lastNumber);
